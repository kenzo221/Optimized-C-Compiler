...
//Parse the current token and return a primitive type enum value, a pointer to any composite type and possibly modify the class of the type.
int parse_type(struct symtable **ctype, int *class) {
  ...
  switch (Token.token) {
    ...
    case T_LONG:
      ...
      //If there is ';' after the parsing then there is no type, so return -1. Example: struct x {int y; int z};
    case T_STRUCT:
      type = P_STRUCT;
...
//Given a type, check that the latest token is a literal of that type. If an integer literal, return this value. If a string literal, return the label number of the string. Do not scan the next token. If type is P_NONE, relax all parse restrictions
int parse_literal(int type) {
  //There is a string literal. Store in memory and return the label
  if (Token.token == T_STRLIT) {
    if (type == pointer_to(P_CHAR) || type == P_NONE)
    return(genglobstr(Text));
  }

  //There is an integer literal. Do some range checking.
...
//Given the type, name and class of a scalar variable, parse any initialisation value and allocate storage for it. Return the variable's symbol table entry.
static struct symtable *scalar_declaration(char *varname, int type, struct symtable *ctype, int class, struct ASTnode **tree) {
  ...
  *tree= NULL;
  ...
  if (Token.token == T_ASSIGN) {
	...
	//Check that the two types are compatible. Change the new type so that the literal parse below works. A 'void *' casstype can be assigned to any pointer type.
...
//Given the type, name and class of an variable, parse the size of the array, if any. Then parse any initialisation value and allocate storage for it. Return the variable's symbol table entry
...
#define TABLE_INCREMENT 10
    ...
    //If the array already has nelems, allocate that many elements in the list. Otherwise, start with TABLE_INCREMENT
    if (nelems != -1)
      maxelems= nelems;
    else
    ...
    while (1) {
      ...
      //Check if there is next value, then parse and add it
	...
	newtype= P_NONE;
      ...
      //Increase the list size if the original size was not set and we have hit the end of the current list
      if (nelems == -1 && i == maxelems) {
        maxelems += TABLE_INCREMENT;
...
//Given a pointer to the new function being declared and a possibly NULL pointer to the function's previous declaration, parse a list of parameters and cross-check them against the previous declaration. Return the count of parameters
...
static struct symtable *function_declaration(char *funcname, int type,
  ...
  //If this is a new function declaration, get a label-id for the end label, and add the function to the symbol table,
  if (oldfuncsym == NULL) {
    endlabel = genlabel();
  ...
  //If this is a new function declaration, update the function symbol entry with the number of parameters. Also copy the parameter list into the function's node.
  if (newfuncsym) {
  ...
  //Do optimizations on the AST tree
  tree= optimize(tree);
...
//Parse composite type declarations: structs or unions. Either find an existing struct/union declaration, or build a struct/union symbol table entry and return its pointer.
static struct symtable *composite_declaration(int type) {
  ...
  //If the next token isn't an LBRACE , this is the usage of an existing struct/union type. Return the pointer to the type.
  ...
  //Set the position of each successive member in the composite type Unions are easy. For structs, align the member and find the next free byte
...
static void enum_declaration(void) {
  ...
  // If the next token isn't a LBRACE, checkthat we have an enum type name, then return
  if (Token.token != T_LBRACE) {
    if (etype == NULL)
...
//Parse the declaration of a variable or function. The type and any following '*'s have been scanned, and we have the identifier in the Token variable. The class argument is the variable's class. Return a pointer to the symbol's entry in the symbol table
static struct symtable *symbol_declaration(int type, struct symtable *ctype,int class, struct ASTnode **tree) {
  struct symtable *sym = NULL;
  char *varname = strdup(Text);

  //Ensure that we have an identifier.  We copied it above so we can scan more tokens in, e.g. an assignment expression for a local variable.
  ident();
...
//Parse a list of symbols where there is an initial type. Return the type of the symbols. et1 and et2 are end tokens.
