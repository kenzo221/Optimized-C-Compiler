...
static int genIF(struct ASTnode *n, int looptoplabel, int loopendlabel) {
  ...
  //Generate two labels: one for the false compound statement, and one for the end of the overall IF statement. When there is no ELSE clause, Lfalse _is_ the ending label!
  Lfalse = genlabel();
  if (n->right)
    Lend = genlabel();
...
static int genSWITCH(struct ASTnode *n) {
  ...
  //Create arrays for the case values and associated labels. Ensure that we have at least one position in each array.
  caseval = (int *) malloc((n->a_intvalue + 1) * sizeof(int));
  caselabel = (int *) malloc((n->a_intvalue + 1) * sizeof(int));

  //Generate labels for the top of the jump table, and the end of the switch statement. Set a default label for the end of the switch, in case we don't have a default.
  Ljumptop = genlabel();
  Lend = genlabel();
  defaultlabel = Lend;
  ...
  for (i = 0, c = n->right; c != NULL; i++, c = c->right) {
    ...
    //Get a label for this case. Store it and the case value in the arrays. Record if it is the default case.
...
//Generate the code to copy the arguments of a function call to its parameters, then call the function itself. Return the register that holds the function's return value.
static int gen_funccall(struct ASTnode *n) {
  struct ASTnode *gluetree = n->left;
  int reg;
  int numargs = 0;

  //If there is a list of arguments, walk this list from the last argument (right-hand child) to the first
  while (gluetree) {
...
int genAST(struct ASTnode *n, int iflabel, int looptoplabel, int loopendlabel, int parentASTop) {
  ...
  //We have some specific AST node handling at the top so that we don't evaluate the child sub-trees immediately
  switch (n->op) {
    case A_IF:
  ...
  switch (n->op) {
    ...
    case A_LE:
    case A_GE:
      //If the parent AST node is an A_IF or A_WHILE, generate a compare followed by a jump. Otherwise, compare registers and set one to 1 or 0 based on the comparison.
    ...
    case A_ASSIGN:
      ...
      //For the '+=' and friends operators, generate suitable code and get the register with the result. Then take the left child, make it the right child so that we can fall into the assignment code.
      switch (n->op) {
	case A_ASPLUS:
	...
	case A_ASMINUS:
      ...
      //Now into the assignment code Are we assigning to an identifier or through a pointer?
      switch (n->right->op) {
	case A_IDENT:
	...
	return (leftreg);
    case A_SCALE:
      //Peephole optimization. Use shift if the scale value is a known power of two
      switch (n->a_size) {
	case 2:
    ...
    case A_TOBOOL:
      //If the parent AST node is an A_IF or A_WHILE, generate a compare followed by a jump. Otherwise, set the register to 0 or 1 based on it's zeroeness or non-zeroeness
