...
//Definitions of structure and enums
...
enum {
  ...
  //Binary operators
  T_ASSIGN, T_ASPLUS, T_ASMINUS, T_ASSTAR, T_ASSLASH, T_LOGOR, T_LOGAND, T_OR, T_XOR, T_AMPER, T_EQ, T_NE, T_LT, T_GT, T_LE, T_GE, T_LSHIFT, T_RSHIFT, T_PLUS, T_MINUS, T_STAR, T_SLASH,
  ...
  //Other keywords
  T_IF, T_ELSE, T_WHILE, T_FOR, T_RETURN, T_STRUCT, T_UNION, T_ENUM, T_TYPEDEF, T_EXTERN, T_BREAK, T_CONTINUE, T_SWITCH, T_CASE, T_DEFAULT,

  //Structural tokens
  T_INTLIT, T_STRLIT, T_SEMI, T_IDENT, T_LBRACE, T_RBRACE, T_LPAREN, T_RPAREN, T_LBRACKET, T_RBRACKET, T_COMMA, T_DOT, T_ARROW, T_COLON
};
...
// Primitive types. The bottom 4 bits is an integer value that represents the level of indirection, e.g. 0= no pointer, 1= pointer, 2= pointer pointer etc.
enum {
...
#define st_endlabel st_posn	//For functions, the end label
  int st_posn;			//For locals, the negative offset from the stack base pointer
