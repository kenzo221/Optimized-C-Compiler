...
static struct ASTnode *funccall(void) {
  ...
  //Check that the identifier has been defined as a function, then make a leaf node for it.
  ...
  //Build the function call AST node. Store the function's return type as this node's type. Also record the function's symbol-id
...
static struct ASTnode *array_access(void) {
  ...
  //Check that the identifier has been defined as an array then make a leaf node for it that points at the base
  ...
  //Return an AST tree where the array's base has the offset added to it, and dereference the element. Still an lvalue at this point.
  left = mkastnode(A_ADD, aryptr->type, left, NULL, right, NULL, 0);
  left = mkastunary(A_DEREF, value_at(left->type), left, NULL, 0);
  return (left);
}

//Parse the member reference of a struct or union and return an AST tree for it. If withpointer is true, the access is through a pointer to the member.
static struct ASTnode *member_access(int withpointer) {
  ...
  //Check that the identifer has been declared as a struct/union or a struct/union pointer
  ...
  //If a pointer to a struct/union, get the pointer's value. Otherwise, make a leaf node that points at the base Either way, it's an rvalue
  if (withpointer) {
...
static struct ASTnode *postfix(void) {
  ...
  //If the identifier matches an enum value, return an A_INTLIT node
...
static struct ASTnode *primary(void) {
  ...
  switch (Token.token) {
  case T_INTLIT:
    //For an INTLIT token, make a leaf AST node for it. Make it a P_CHAR if it's within the P_CHAR range
  ...
  case T_STRLIT:
    //For a STRLIT token, generate the assembly for it. Then make a leaf AST node for it. id is the string's label.
    id = genglobstr(Text);
  ...
  case T_LPAREN:
    ...
    switch (Token.token) {
      case T_IDENT:
        //We have to see if the identifier matches a typedef. If not, treat it as an expression.
    ...
    //We now have at least an expression in n, and possibly a non-zero type in type if there was a cast. Skip the closing ')' if there was no cast.
    if (type == 0)
      rparen();
    else
...
struct ASTnode *prefix(void) {
  ...
  case T_MINUS:
    ...
    //Prepend a A_NEGATE operation to the tree and make the child an rvalue. Because chars are unsigned, also widen this to int so that it's signed
    tree->rvalue = 1;
...
struct ASTnode *binexpr(int ptp) {
  ...
  //While the precedence of this token is more than that of the previous token precedence, or it's right associative and equal to the previous token's precedence
  while ((op_precedence(tokentype) > ptp) ||
	...
	fatal("Incompatible expression in assignment");
      ...
      //Make an assignment AST tree. However, switch left and right around, so that the right expression's  code will be generated before the left expression
      ltemp = left;
      left = right;
      right = ltemp;
    } else {

      //We are not doing an assignment, so both trees should be rvalues Convert both trees into rvalue if they are lvalue trees
      left->rvalue = 1;
      right->rvalue = 1;
    ...
    //Update the details of the current token. If we hit a terminating token, return just the left node
    tokentype = Token.token;
    if (tokentype == T_SEMI || tokentype == T_RPAREN ||
	tokentype == T_RBRACKET || tokentype == T_COMMA ||
        tokentype == T_COLON) {
