...
static struct ASTnode *if_statement(void) {
  ...
  //Parse the following expression and the ')' following. Force a non-comparison to be boolean the tree's operation is a comparison.
  condAST = binexpr(0);
  ...
  //If we have an 'else', skip it and get the AST for the statement
...
static struct ASTnode *while_statement(void) {
  ...
  //Parse the following expressionand the ')' following. Force a non-comparison to be boolean the tree's operation is a comparison.
  condAST = binexpr(0);
  if (condAST->op < A_EQ || condAST->op > A_GE)
    condAST = mkastunary(A_TOBOOL, condAST->type, condAST, NULL, 0);
  rparen();

  //Get the AST for the statement. Update the loop depth in the process
  Looplevel++;
...
static struct ASTnode *for_statement(void) {
  ...
  //Get the condition and the ';'. Force a non-comparison to be boolean the tree's operation is a comparison.
  condAST = binexpr(0);
  if (condAST->op < A_EQ || condAST->op > A_GE)
    condAST = mkastunary(A_TOBOOL, condAST->type, condAST, NULL, 0);
  semi();
...
static struct ASTnode *switch_statement(void) {
  ...
  while (inloop) {
	...
	if (Token.token==T_DEFAULT) {
	  ...
	  //Walk the list of existing case values to ensure that there isn't a duplicate case value
	...
	//Build a sub-tree with the compound statement as the left child and link it in to the growing A_CASE tree
	if (casetree==NULL) {
  ...
  //We have a sub-tree with the cases and any default. Put the case count into the A_SWITCH node and attach the case tree.
...
static struct ASTnode *single_statement(void) {
  ...
  switch (Token.token) {
    case T_LBRACE:
      ...
      return(stmt);
    case T_IDENT:
      //We have to see if the identifier matches a typedef. If not, treat it as an expression. Otherwise, fall down to the parse_type() call
...
//Parse a compound statement and return its AST. If inswitch is true, we look for a '}', 'case' or 'default' token to end the parsing. Otherwise, look for just a '}' to end the parsing.
struct ASTnode *compound_statement(int inswitch) {
  ...
  while (1) {
    ...
    //For each new tree, either save it in left if left is empty, or glue the left and the new tree together
    if (tree != NULL) {
      if (left == NULL)
	left = tree;
      else
