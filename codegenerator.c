...
//Flag to say which section output is in
...
//Given a scalar type, an existing memory offset (which hasn't been allocated to anything yet) and a direction (1 is up, -1 is down), calculate and return a suitably aligned memory offset for this scalar type. This could be the original offset, or it could be above/below the original
int cgalign(int type, int offset, int direction) {
  int alignment;

  //Align chars on any offset and align ints/pointers on a 4-byte alignment
  switch (type) {
    case P_CHAR:
      return (offset);
    case P_INT:
  ...
  //Align int or long on a 4-byte offset
  alignment = 4;
...
void cgfuncpreamble(struct symtable *sym) {
  ...
  //For the remainder, if they are a parameter then they are already on the stack. If only a local, make a stack position.
...
//Load a value from a variable into a register.Return the number of the register. If the operation is pre- or post-increment/decrement, also perform this action.
...
//Load a value from a local variable into a register. Return the number of the register. If the operation is pre- or post-increment/decrement, also perform this action
...
//Call a function with the given symbol id Pop off any arguments pushed on the stack Return the register with the result
int cgcall(struct symtable *sym, int numargs) {
  ...
  if (numargs > 6)
    fprintf(Outfile, "\taddq\t$%d, %%rsp\n", 8 * (numargs - 6));
  //copy the return value into our register
  fprintf(Outfile, "\tmovq\t%%rax, %s\n", reglist[outr]);
  return (outr);
}

//Given a register with an argument value, copy this argument into the argposnparameter in preparation for a future function call
void cgcopyarg(int r, int argposn) {

  //If this is above the sixth argument, simply push the register on the stack
  if (argposn > 6) {
...
void cgglobsym(struct symtable *node) {
  ...
  //Get the size of the variable (or its elements if an array) and the type of the variable
  ...
  for (i=0; i < node->nelems; i++) {
    ...
    switch (size) {
      ...
      case 4:
        ...
        break;
      case 8:
        //Generate the pointer to a string literal. Treat a zero value as actually zero, not the label L0
...
int cgderef(int r, int type) {
  //Get the type that value is pointing at
...
//Generate a switch jump table and the code to load the registers and call the switch() code
void cgswitch(int reg, int casecount, int toplabel,
  ...
  //If there is no cases, create one case which points to the default case
  if (casecount == 0) {
    caseval[0] = 0;
