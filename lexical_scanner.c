...
//Lexical scanning for tokens and keywords
...
static int next(void) {
  ...
  if (Putback) {		//Putback a character if required
    c = Putback;		
    Putback = 0;
    return (c);
  }
  ...
  while (c == '#') {		//Pre-processor statement
    scan(&Token);		
    ...
    if (Text[0] != '<') {		//If this is a real filename and not the one we have now save it. Then update the line number
      if (strcmp(Text, Infilename))	
	Infilename = strdup(Text);	 
      Line = l;
    }

    while ((c = fgetc(Infile)) != '\n');	//Skip to the end of the line and get the next character
    c = fgetc(Infile);	
  }
...
static int scanstr(char *buf) {
  ...
  for (i = 0; i < TEXTLEN - 1; i++) {
    //Get the next char and append to buf. Return when we hit the ending double quote
    if ((c = scanch()) == '"') {
      buf[i] = 0;
      return (i);
    }
...
//Keyword scanning optimization. Switch on the first letter so that we don't have to waste time strcmp()ing against all the keywords.
...
//List of token strings,  debugging purposes
char *Tstring[] = {
  "EOF", "=", "+=", "-=", "*=", "/=", "||", "&&", "|", "^", "&", "==", "!=", ",", ">", "<=", ">=", "<<", ">>", "+", "-", "*", "/", "++", "--", "~", "!", "void", "char", "int", "long", "if", "else", "while", "for", "return", "struct", "union", "enum", "typedef", "extern", "break", "continue", "switch", "case", "default", "intlit", "strlit", ";", "identifier", "{", "}", "(", ")", "[", "]", ",", ".", "->", ":"
};
