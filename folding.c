...
//Optimization method for AST tree called Constant folding

//Fold an AST tree with a binary operator and two A_INTLIT children. Return either the original tree or a new leaf node.
...
//Fold an AST tree with a unary operator and one INTLIT children. Return either the original tree or a new leaf node.
...
//Constant folding optimization. Perform constant folding on AST tree with the root node n
static struct ASTnode *fold(struct ASTnode *n) {

  if (n == NULL)
    return (NULL);

  //Fold on the left child, then on the right child
  n->left = fold(n->left);
...
//Optimize an AST tree by constant folding in all sub-trees
struct ASTnode *optimize(struct ASTnode *n) {
  n = fold(n);
  return (n);
}
